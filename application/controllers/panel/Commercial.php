...
class Commercial extends CI_Controller {
    ...
    private $name = "commercial";
    private $table = "commercial";
    private $icon = 'fa-percent';
    private $redirect = 'panel/commercial';
    ...
    public function index()
    {
        if (access($this->name,'index')) {
            ...
            $this->template->load('panel/template','panel/commercial/index', $data);
        }else{
            error_404();
        }
    }
    ...
    public function get()
    {
        if (access($this->name,'list')) {
            $fetch_data = $this->main->make_datatables('panel/CommercialModel');
    		$sr = $_POST['start'] + 1;
            $data = array();  
            ...
            {  
                ...
                if ($this->session->userdata('role') != 'builder') {
                    if ($row->featured == 0) {
                    $sub_array[] = '<a onclick="return confirm(\'Are You Sure ?\')" href="'.base_url('panel/commercial/featured/').$row->id.'/1" class="text-danger"><i class="fa fa-square-o fa-lg"></i></a><br>';
                    }else{
                        $sub_array[] = '<a onclick="return confirm(\'Are You Sure ?\')" href="'.base_url('panel/commercial/featured/').$row->id.'/0" class="text-success"><i class="fa fa-check-square-o fa-lg"></i></a>';
                    }
                }
                $sub_array[] = '<div class="table_form">
                <a class="btn btn-success" href="'.base_url('panel/').'commercial/view?id='.my_crypt($row->id,'e').'"><i class="fa fa-eye "></i></a>
                <a class="btn btn-primary" href="'.base_url('panel/').'commercial/update?id='.my_crypt($row->id,'e').'"><i class="fa fa-pencil-square-o "></i></a>
                <form action="'.base_url('panel/').'commercial/delete" method="POST"><input type="hidden" name="id" value="'.$row->id.'"><button class="delete btn btn-danger"><i class="fa fa-trash"></i></button>
                </form></div>
                ';
        	...
        	$output = array(  
                ...
                "recordsFiltered"   =>     $this->main->get_filtered_data('panel/CommercialModel'),  
                "data"              =>     $data  
            );  
     ...
     public function view()
    {
        if (access($this->name, 'add') && !empty($this->input->get('id'))) { 
            ...
            $data['property'] = $this->fmain->get($this->table, 'title, price, area, type, parking, address, state, city, pincode, details, features, builder, plan_image, multi_image,c_area,b_area',['id'=>$id]);
            $this->template->load('panel/template','panel/commercial/view', $data);
        }else{
            error_404();
        }
    }
    ...
    {   
        if (access($this->name,'add')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                $this->template->load('panel/template','panel/commercial/add', $data);
            }else{
    ...
    {   
        if (access($this->name, 'add')) { 
            ...
            $this->template->load('panel/template','panel/commercial/upload', $data);
        }else{
            error_404();
        }
    }
    ...
    {   
        if (access($this->name, 'add')) { 
            $config['upload_path']= "./assets/img/properties/commercial/"; 
            $config['allowed_types']='jpg|jpeg';
    ...
    {   
        if (access($this->name, 'update')) { 
            $multi_image = $this->main->check($this->table, ['id'=>$id], 'multi_image');
            $image_path = "./assets/img/properties/commercial/".$img;
            unlink($image_path);
    ...
    public function update()
    {
        if (access($this->name, 'update') && !empty($this->input->get('id'))) { 
            ...
            $data['property'] = $this->fmain->get($this->table, 'id,title, price, area, type, parking, address, state, city, pincode, details, features, builder, plan_image, multi_image,c_area,b_area',['id'=>$id]);

            $this->form_validation->set_rules($this->validate);
            if ($this->form_validation->run() == FALSE) {
                $this->template->load('panel/template','panel/commercial/edit', $data);
            }else{
                $post = $this->input->post();
                $post['features'] = implode(',', $post['features']);
